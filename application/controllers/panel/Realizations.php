...
class Realizations extends CI_Controller {
	...
	public function form($type, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['realizations_filters'] = $this->back_m->get_all_where('realizations_filters', 'realization_id', $id);
	...
	public function action($type, $table, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			if($type == 'insert') {
				...
				$last = $this->back_m->get_last($table);
			} else {
				$this->back_m->update($table, $insert, $id);
				$this->session->set_flashdata('flashdata', 'Rekord został zaktualizowany!');
				$last = $this->back_m->get_one($table, $id);   
			}
			if(isset($_POST['filters'])) {
				$filters = $this->back_m->get_all_where('realizations_filters', 'realization_id', $last->id);
				foreach ($filters as $filter_delete) {
					$this->back_m->delete('realizations_filters', $filter_delete->id);
				}
				foreach($_POST['filters'] as $filter) {

					$this->back_m->insert('realizations_filters', ['filter_id' => $filter, 'realization_id' => $last->id]);
				}
			}

...
class Services_sections extends CI_Controller {
	...
	public function show($item_id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			if (!$this->db->table_exists('services_sections')){
				$this->base_m->create_table('services_sections');
			}
            // DEFAULT DATA
			$data = loadDefaultData();

			$data['rows'] = $this->back_m->get_all_where('services_sections', 'item_id', $item_id);
			echo loadSubViewsBack($this->uri->segment(2), 'index', $data);
		} else {
			redirect('panel');
		}
